...
def _build_exec_sql_step(
    ...
    ET.SubElement(step, "name").text = step_name
    ET.SubElement(step, "type").text = "ExecSQL"
    ET.SubElement(step, "description")
    ET.SubElement(step, "distribute").text = "Y"
    ET.SubElement(step, "custom_distribution")
    ET.SubElement(step, "copies").text = "1"
    ...
    ET.SubElement(step, "sql").text = sql
    ET.SubElement(step, "execute_each_row").text = "N"
    ET.SubElement(step, "single_statement").text = "N"
    ET.SubElement(step, "replace_variables").text = "Y"
    ET.SubElement(step, "quoteString").text = "Y"
...
def generate_ktr(
    ...
    drop_statements: Optional[list[str]] = None,
    ...
    for block in sql_blocks:
        ...
        if drop_sql and table:
            ...
            drop_step = _build_exec_sql_step(
                step_name=drop_step_name,
                sql=drop_sql,
                connection_name=connection_name,
                x=dx,
                y=dy,
            )
            root.append(drop_step)
            step_names.append(drop_step_name)
